  ...
  from (
    ...
    from(
      select
        ...
        -- check if the first acl entry is for the group we're searching for
        split_part(split_part(array_to_string(c.relacl, '|'), '|', 1), 'group ' || g.groname || '=', 2)  as first_acl,
        -- check if any acl entry except the first is for the group we're searching for
        split_part(split_part(array_to_string(c.relacl, '|'), '|group ' || g.groname || '=', 2), '|', 1) as other_acl,
        c.relacl
